...
class vv:
    ...
    @classmethod
    def from_init_conds(cls, x,v,m):
        return cls(x,v,np.zeros_like(v),m)
    ...
    def get_grad(self):
        return self.harmonic_grad()

    def harmonic_grad(self):
        return 2*self.x

    #def distance(self):
        #return np.sqrt(np.sum(np.square(self.x),axis=1))
...
class grow_vv:
    ...
    def write_out_geometries(self,file):
        ...
        for i in range(len(self.t_history)):
            geoms.append(xyz.Geometry(len(self.elements),f"t={self.t_history[i]}; generated by grow_vv",self.elements,self.x_history[i]))
        geoms = xyz.combinedGeoms(geoms)
        geoms.write_all(file)



# There was a coding sin, but I don't know if it's copy pasting and editting, or if its the behemoth I left behind
@contextmanager
